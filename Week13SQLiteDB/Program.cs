...
static void InsertCustomer(SQLiteConnection myConnection)
{
    ...
    Console.WriteLine("Enter day of birth (dd-mm-yyyy");
    ...
    Console.WriteLine($"Rowes inserted: {rowInserted}");
...
static void RemoveCustomer(SQLiteConnection myConnection)
{
    SQLiteCommand command;
    string idToDelete;
    Console.WriteLine("Enter an ID a custumer to delete");
    idToDelete = Console.ReadLine();

    command = myConnection.CreateCommand();
    command.CommandText = $"DELETE FROM customer WHERE rowid = {idToDelete}";
    int rowRemoved = command.ExecuteNonQuery();
    Console.WriteLine($"{rowRemoved} was removed from the table custumer.");
